...
- `translation`: use the sequence ID to query the `annotation` table where `ano_type_id` is "Translation" (trm_id = 761) and the `ano_linkfrom_ids` column array contains the sequence ID in the format of `seq:id` (e.g. `seq:1`). If there is a match, use the `ano_text` column as the translation value. If there are multiple matches, concatenate the `ano_text` values with a separator (e.g. "; "). If there is no match, set the `translation` property to null.
- `chaya`: use the sequence ID to query the `annotation` table where `ano_type_id` is "Chaya" (trm_id = 1421) and the `ano_linkfrom_ids` column array contains the sequence ID in the format of `seq:id` (e.g. `seq:1`). If there is a match, use the `ano_text` column as the chaya value. If there are multiple matches, concatenate the `ano_text` values with a separator (e.g. "; "). If there is no match, set the `chaya` property to null.
